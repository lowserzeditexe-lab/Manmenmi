...
- **OBSERVED :** MANMENMI valide un micro-sous-ensemble Shader IR structuré,
  traduit en HLSL puis compilé par `D3DCompile` sur DX12 Windows ; le triangle
  M3 produit toujours des pixels non-clear après readback privé.
- **DOCUMENTED :** cette IR est un corpus structuré interne, pas un parser de
  blob GX2 réel.
- **UNKNOWN :** format, encodage, opcodes et conventions complètes des shaders
  GX2/Latte ; aucune compatibilité de shader Wii U n'est déduite du test M6.
...
- **DOCUMENTED [S4] :** la recherche wut établit des familles GX2 de surface et
  texture, mais pas dans ce dépôt un enum de format ou une sémantique de layout
  suffisante pour une conversion console normative.
- **INFERRED + OBSERVED :** M8 utilise une représentation hôte normalisée
  `rgba8_unorm`/`linear`, convertie vers la Texture M7 puis validée par draw et
  readback DX12. Cela prouve le pont logiciel, pas le format Wii U réel.
- **UNKNOWN :** correspondance exacte des formats, tiling, swizzle, alignements,
  cache et mémoire GX2 ; aucun de ces comportements n'est implémenté.

### M8 evidence revision (2026-09-19)

This revision supersedes the normalized host-only M8 text immediately above.

- **DOCUMENTED [S4] :** the pinned WUT `GX2Surface` declaration contains
  `width`, `height`, `format`, `use`, `tileMode`, `swizzle`, `alignment`, and
  `pitch`. Its enums name `GX2_SURFACE_FORMAT_UNORM_R8_G8_B8_A8` (`0x01a`),
  `GX2_SURFACE_USE_COLOR_BUFFER` (`1 << 1`), and
  `GX2_TILE_MODE_LINEAR_ALIGNED` (`1`).
- **INFERRED :** projecting the documented RGBA UNORM name to MANMENMI
  `rgba8_unorm` is the smallest semantic mapping. DX12 renders to the resulting
  target, but that observation does not prove console byte order.
- **OBSERVED :** `unit.gx2_texture` rejects invalid dimensions, format, usage,
  and layout; `integration.gx2_texture` records the backend-neutral descriptor;
  `runtime.integration_m8` draws with M6 and confirms a DX12 readback pixel.
- **UNKNOWN :** byte order, pitch calculation, alignment, swizzle, tile
  addressing, cache behavior, memory allocation, and data upload for every GX2
  tile mode. M8 therefore carries `LINEAR_ALIGNED` only as descriptor metadata.
